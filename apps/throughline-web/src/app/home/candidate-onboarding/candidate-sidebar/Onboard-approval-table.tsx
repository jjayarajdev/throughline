...
/** Final-onboarding approvals: accept / reject a candidate's onboarding with a comment. */
export function OnboardApprovalTable() {
  ...
  const { data: candidatesResponse, isLoading, error, refetch: reFetchData } = useQuery({
    queryKey: ["onboardApproval", t.query],
    queryFn: () =>
  ...
  const confirm = async (id: string, candidateId: string, approve: boolean, comments: string) => {
    ...
    try {
      const res = await api.post(`/CandidateForm/candidate-onboarding-confirmation`, { candidateId, approve, comments, candidatePersonalDetailsId: Number(id) });
      if (res.status === 200) {
  ...
  return (
    <>
      <DataTable<Candidate>
        storageKey="onboard-approval"
        rowKey={(c) => c.candidateBinId ?? c.candidatePersonalDetailsId ?? c.id}
        columns={columns}
