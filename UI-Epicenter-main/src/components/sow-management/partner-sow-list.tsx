...
export default function PartnerSowManagement() {
  ...
  const [columns, setColumns] = useState<Column[]>([
    { id: "expand", label: "Sow", visible: true },
    { id: "partnerCode", label: "Partner ID", visible: true },
    { id: "nickname", label: "Partner", visible: true, sortable: true },
    {
  ...
  const {
    ...
    queryKey: ["partners", currentPage, searchColumn, searchText, statusId, pageSize],
    queryFn: () =>
  ...
  // Get active SOWs
   const { data: PARTNER_STATUS = [] } = useQuery({
    ...
    enabled: true,
    select: (data) => [
      ...data.filter((item) => item.id !== 19004)
    ],
  });
  const { data: activeSowResponse, isLoading: isActiveSowLoading } = useQuery({
    queryKey: ["sowDetails", expandedPartnerId, 1],
    queryFn: () => partnerApi.getSowDetails(1, expandedPartnerId, {}),
    enabled: !!expandedPartnerId,
  });

  // Get expiring SOWs
  const { data: expiringSowResponse, isLoading: isExpiringSowLoading } =
    useQuery({
      queryKey: ["sowDetails", expandedPartnerId, 2],
      queryFn: () => partnerApi.getSowDetails(2, expandedPartnerId, {}),
      enabled: !!expandedPartnerId,
    });

  // Get inactive SOWs
  const { data: inactiveSowResponse, isLoading: isInactiveSowLoading } =
    useQuery({
      queryKey: ["sowDetails", expandedPartnerId, 3],
      queryFn: () => partnerApi.getSowDetails(3, expandedPartnerId, {}),
      enabled: !!expandedPartnerId,
    });
  ...
  const partners = (partnersResponse?.data?.items || []).map(({ stateId, countryId, cityId, contactMatrices, engagements, capabilitiesDeckDocuments, ...rest }: any) => rest);
  ...
  const isLoadingSows =
    isActiveSowLoading || isExpiringSowLoading || isInactiveSowLoading;
                      ...
                      {visibleColumns.map((column) => (
                        <TableCell key={`${partner.id}-${column.id}`}>
                         ...
                         <ul className="list-disc pl-4">
                            {[...new Set((partner?.engagementTypeName || "")
                            .split(",")
                            .map((item:string) => item.trim()))].map((item:any, idx) => (
                        ...
                        >
                          <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-inner">
                            <Tabs defaultValue="active" className="w-full">
                              <TabsList className="w-full h-12 bg-gradient-to-r from-teal-100 to-teal-50 dark:from-teal-900 dark:to-teal-800 rounded-t-lg">
                                <TabsTrigger
                                  value="active"
                                  className="data-[state=active]:bg-cyan-500 data-[state=active]:text-white dark:data-[state=active]:bg-cyan-900"
                                >
                                  Active (
                                  {activeSowResponse?.data?.items?.length || 0})
                                </TabsTrigger>
                                <TabsTrigger
                                  value="expiring"
                                  className="data-[state=active]:bg-teal-500 data-[state=active]:text-white dark:data-[state=active]:bg-teal-950"
                                >
                                  Up for Renewal{" "}
                                  <span className="text-red-500">
                                    (
                                    {expiringSowResponse?.data?.items?.length ||
                                      0}
                                    )
                                  </span>
                                </TabsTrigger>
                                <TabsTrigger
                                  value="inactive"
                                  className="data-[state=active]:bg-gray-500 data-[state=active]:text-white dark:data-[state=active]:bg-gray-900"
                                >
                                  Inactive (
                                  {inactiveSowResponse?.data?.items?.length ||
                                    0}
                                  )
                                </TabsTrigger>
                              </TabsList>
                              ...
                              {isLoadingSows ? (
                                ...
                                <>
                                  ...
                                  <TabsContent value="expiring">
                                    <SowTable
                                      data={
                                        expiringSowResponse?.data?.items || []
                                      }
                                    />
                                  </TabsContent>
                                  ...
                                  <TabsContent value="inactive">
                                    <SowTable
                                      data={
                                        inactiveSowResponse?.data?.items || []
                                      }
                                    />
                                  </TabsContent>
                                </>
