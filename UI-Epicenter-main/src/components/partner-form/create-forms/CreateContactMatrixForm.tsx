...
export default function CreateContactMatrixForm({
  onNext,
  onPrevious,
}: ContactMatrixFormProps) {
  ...
  const { data: contactTypes = [] } = useQuery({
    ...
    queryFn: () =>
      dropdownApi.fetchDropdown(MasterTypes.CONTACT_MATRIX_CONTACT_TYPE),
    enabled: mounted,
  });
  ...
  const updateContactMutation = useMutation({
    mutationFn: (values: ContactMatrixPayload) =>
      partnerApi.updateContactMatrix(selectedContact.id, values),
    onSuccess: async (data) => {
    ...
    onError: (error:any) => {
        const message =
            error?.response?.data?.message ||
            error?.message ||                 
           "Failed to update contactMatrix";
      ...
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
          ...
          {remainingContacts > 0 && (
            <Alert>
              <AlertDescription>
                Please add {remainingContacts} more contact
