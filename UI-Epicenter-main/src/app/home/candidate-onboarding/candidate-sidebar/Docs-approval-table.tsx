...
export function DocsApprovalTable() {
 ...
 const [columns,setColumns] = useState([
    ...
    {id:"ndaDoc",label:"NDA DOC",visible:true},
    {id:"cdaDoc",label:"CDA DOC",visible:true},
   {
      id:"intakeStatusName",
  ...
  const {
    ...
    queryKey: ["docsApproval", currentPage, searchColumn, searchText,pageSize],
    queryFn: () =>
...
const [loader,setLoader] = useState(false);
  const handleApprove = async (id:string,candidateId: string, comments: string) => {
    ...
    try {
      ...
      const res = await api.post(`/CandidateForm/candidate-bgv-confirmation`,payload);
      if (res.status === 200) {
  ...
  const handleReject = async (id: string,candidateId:string, comments: string) => {
    try {
      const res = await api.post(
        `/CandidateForm/candidate-bgv-confirmation`,
        {
          candidateId: candidateId,
          approve:false,
          comments: comments,
...
const isOlderThan72Hours = (doc: any) => {
  ...
  const diffMs = now.getTime() - docDateTime.getTime();
          ...
          <TableBody>
            {isLoading ? (
              ...
              candidates.map((candidate: Candidate) => (
                <TableRow className={
                 isOlderThan72Hours(candidate?.cdaDoc) &&
                 isOlderThan72Hours(candidate?.ndaDoc)
                  ? "bg-red-100"
                : ""
                    ...
                    <TableCell key={`${candidate.id}-${column.id}`}>
                      {column.id === "hrqId" ? (
                       ...
                       column.id === "employeeId" ? (
                        ...
                        <ResumePreview 
                         url={candidate?.ndaDoc?.attachmentURL}
                            fileName={`${candidate?.ndaDoc?.attachmentName}'s`}
                        />
                       
                      ) :column.id === "cdaDoc" ? (
                        
                        <ResumePreview 
                         url={candidate?.cdaDoc?.attachmentURL}
                            fileName={`${candidate?.cdaDoc?.attachmentName}'s`}
                        />
