...
-- =====================================================================
--  fastalent · only the tables that change. Everything else
--  (company_wallets, wallet_transactions, earnings, payout_batches,
--  payout_requests, notifications, role_invitations, hiring_metrics,
--  anomaly_alerts, ai_usage_logs, feature_flags, platform_settings,
--  country_configs, audit_logs) moves over unchanged.
-- =====================================================================
CREATE TYPE fastalent.role_visibility   AS ENUM ('open', 'preferred', 'invite_only');
CREATE TYPE fastalent.role_type         AS ENUM ('regular', 'headhunting');
CREATE TYPE fastalent.payout_type       AS ENUM ('per_shortlist', 'per_hire', 'hybrid');
CREATE TYPE fastalent.payout_mode       AS ENUM ('flat', 'percentage');
CREATE TYPE fastalent.submission_status AS ENUM ('submitted', 'shortlisted', 'interview', 'hired', 'joined', 'rejected', 'withdrawn');
CREATE TYPE fastalent.reputation_tier   AS ENUM ('bronze', 'silver', 'gold', 'platinum');

-- Was: User (auth columns only; identity moved to core.users)
CREATE TABLE fastalent.user_auth (
  user_id       uuid          PRIMARY KEY REFERENCES core.users,
  role          text          NOT NULL CHECK (role IN ('recruiter','company','admin')),
  password_hash text,
...
-- Was: RecruiterProfile (name/phone moved to core.users; agency link is core.organisation_members)
CREATE TABLE fastalent.recruiter_profiles (
  user_id       uuid          PRIMARY KEY REFERENCES core.users,
  agency_organisation_id uuid REFERENCES core.organisations,   -- NULL = independent gig recruiter
  bio           text,
  ...
  reputation_tier fastalent.reputation_tier NOT NULL DEFAULT 'bronze',
  total_placements int        NOT NULL DEFAULT 0,
  successful_placements int   NOT NULL DEFAULT 0,
  last_submission_at timestamptz
);

-- Was: CompanyProfile (company identity moved to core.organisations)
CREATE TABLE fastalent.company_settings (
  organisation_id uuid        PRIMARY KEY REFERENCES core.organisations,
  owner_user_id   uuid        NOT NULL REFERENCES core.users,
  default_commission_pct numeric(5,2),
  currency        char(3)     NOT NULL
);

-- Was: Role (requisition columns moved to core.jobs)
CREATE TABLE fastalent.job_marketplace (
  job_id        uuid          PRIMARY KEY REFERENCES core.jobs,
  role_type     fastalent.role_type NOT NULL DEFAULT 'regular',
  visibility    fastalent.role_visibility NOT NULL DEFAULT 'open',
  marketplace_status text     NOT NULL DEFAULT 'draft'
                              CHECK (marketplace_status IN ('draft','submitted','published','rejected','paused','closed','filled')),
  payout_type   fastalent.payout_type NOT NULL,
  shortlist_payout_mode fastalent.payout_mode,
  shortlist_payout_value numeric(14,2),
  hire_payout_mode      fastalent.payout_mode,
  hire_payout_value     numeric(14,2),
  platform_commission_pct numeric(5,2),
  vendor_benchmark_pct    numeric(5,2),
  max_submissions   int       NOT NULL DEFAULT 50,
  max_per_recruiter int       NOT NULL DEFAULT 3,
...
-- Was: Submission (candidate columns moved to core.people / core.applications)
CREATE TABLE fastalent.submissions (
  id            uuid          PRIMARY KEY DEFAULT gen_random_uuid(),
  application_id uuid         NOT NULL UNIQUE REFERENCES core.applications,
  recruiter_user_id uuid      NOT NULL REFERENCES core.users,
  status        fastalent.submission_status NOT NULL DEFAULT 'submitted',
  cover_note    text,
  current_location text,
  accepted_ctc  numeric(14,2),
...
CREATE INDEX ON fastalent.submissions (recruiter_user_id, status);

-- Was: JdCrux (now keyed by core job)
CREATE TABLE fastalent.jd_crux (
  job_id        uuid          PRIMARY KEY REFERENCES core.jobs,
  status        text          NOT NULL,
  key_skills    text[]        NOT NULL DEFAULT '{}',
  key_skill_ids int[]         NOT NULL DEFAULT '{}',        -- resolved against core.skills
...
GRANT USAGE ON SCHEMA fastalent TO app_fastalent;
GRANT ALL ON ALL TABLES IN SCHEMA fastalent TO app_fastalent;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA fastalent TO app_fastalent;

-- =====================================================================
--  throughline · only the tables that change. Everything else
--  (InterviewSlots, InterviewSlotAllocationHistory, CandidateInterviewFeedBack,
--  InterviewRounds, PanelHistory, Calibration, Feedback*, JoiningRescheduleHistory,
--  CandidateOnboardingDetails, CandidateRateCard, CandidateBin*, SOW*, PO*,
--  Engagements, PartnerEmpanel, PartnerApprovalHistory, EmailTemplates,
--  M_Configuration, M_SearchColumn, M_MenuItem, M_AccessControl) moves over
--  unchanged except that PartnerId → organisation_id, UserId → user_id (uuid),
--  and skill/location int lists become FKs to core taxonomy.
-- =====================================================================

-- Was: Users + UserRole (identity moved to core.users)
CREATE TABLE throughline.user_roles (
  user_id       uuid          NOT NULL REFERENCES core.users,
  role_id       smallint      NOT NULL,                    -- ADMIN=1 … BETMember=11 (existing Role table)
  domain_id     smallint      REFERENCES core.domains,     -- for DomainManager
  organisation_id uuid        REFERENCES core.organisations, -- for PARTNER users
  PRIMARY KEY (user_id, role_id)
);

-- Was: Partners (identity moved to core.organisations kind='vendor')
CREATE TABLE throughline.partner_details (
  organisation_id uuid        PRIMARY KEY REFERENCES core.organisations,
  partner_category_id int     REFERENCES core.lookups,
  partner_tier_id     int     REFERENCES core.lookups,
  servicing_country_id smallint REFERENCES core.countries,
  nickname      text,
  start_date    date,
  approved_status int,
  approved_by_user_id uuid    REFERENCES core.users,
  approved_at   timestamptz,
  is_empanel_enabled boolean  NOT NULL DEFAULT false,
  last_activated_at timestamptz,
  capabilities_deck_document_ids uuid[] NOT NULL DEFAULT '{}'
);

-- Was: Hiring + JobDetails (requisition columns moved to core.jobs)
CREATE TABLE throughline.hiring_requests (
  job_id        uuid          PRIMARY KEY REFERENCES core.jobs,
  record_type_id int,
  engagement_type_id int      REFERENCES core.lookups,
  hiring_type_id  int         REFERENCES core.lookups,
  hiring_status_id int        NOT NULL,                    -- 12001 NEW … 12008 CANCELLED
  previous_hiring_status_id int,
  approval_status_id int,
  rm_owner_user_id uuid       REFERENCES core.users,
  is_rm_owner_accepted boolean,
  rm_owner_accepted_at timestamptz,
  rm_owner_comments text,
  bet_approver_user_id uuid   REFERENCES core.users,
  request_approver_user_id uuid REFERENCES core.users,
  approver_comments text,
  approver_updated_at timestamptz,
  is_parent_hrq boolean       NOT NULL DEFAULT false,
  parent_job_id uuid          REFERENCES core.jobs,
  referred_job_id uuid        REFERENCES core.jobs,
  rcms_project_id text,
  rcms_resource_request_id text,
  project_name  text,
  business_id   int,
  project_duration_months smallint,
  request_start_date date,
  skip_screening boolean      NOT NULL DEFAULT false,
  resource_type_id int        REFERENCES core.lookups,
  badge_rec_id  int,
  mandatory_certification text,
  on_hold jsonb,                                           -- the OnHold* column group
  cancelled_at  timestamptz
);

-- Was: Candidate/CandidateForms (person + application columns moved to core)
CREATE TABLE throughline.candidates (
  application_id uuid         PRIMARY KEY REFERENCES core.applications,
  candidate_code text          UNIQUE,                     -- 'HRQ1572001' — kept for users and old links
  intake_status_id int,
  previous_intake_status_id int,
  candidate_status_id int,
  screening_status text,
  is_screening_completed boolean NOT NULL DEFAULT false,
  ...
  resource_type_id int        REFERENCES core.lookups,
  pc_lifecycle_id int,
  requested_microsoft_account boolean,
  considered_for_future boolean NOT NULL DEFAULT false,
  is_freezed boolean          NOT NULL DEFAULT false,
  freeze_reason text,
  freezed_at timestamptz,
  is_transferred boolean      NOT NULL DEFAULT false,
  original_job_id uuid        REFERENCES core.jobs,
  milestones jsonb,                                        -- offered/accepted/declined/joined/rejected/on-hold timestamps
  comments text,
  approved_by_user_id uuid    REFERENCES core.users,
  approved_at timestamptz
);

GRANT USAGE ON SCHEMA throughline TO app_throughline;
GRANT ALL ON ALL TABLES IN SCHEMA throughline TO app_throughline;
GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA throughline TO app_throughline;
